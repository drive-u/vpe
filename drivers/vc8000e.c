					...
					u32 *core_id, u32 *irq_status)
{
	...
	ret = wait_event_interruptible_timeout(tvce->enc_wait_queue,
					    vce_check_irq(tvce, core_id, irq_status),
					    VC8000E_TIMEOUT);
	if (!ret) {
		trans_dbg(tvce->tdev, TR_ERR,
...
static int vce_reserve_core(struct cb_tranx_t *tdev,
...
static int vce_release_core(struct cb_tranx_t *tdev, u32 core_id,
				int mode, struct file *filp)
{
	...
	if (tvce->core[id].filp == NULL) {
		trans_dbg(tdev, TR_ERR,
			"vc8000e: %s core:%d is not reserve, status=%s\n",
			__func__, id, core_status[tvce->core[id].core_status]);
	...
	for (id = 0; id < VCE_MAX_CORES; id++) {
		if (tvce->core[id].filp == filp) {
			trans_dbg(tdev, TR_NOTICE,
				...
				__func__, id, filp);
			vce_release_core(tdev, 1<<id, ABNORM_EXIT, filp);
		}
	}
}
...
irqreturn_t vce_isr(int index, void *data)
{
	...
	if (RSV_FLAG != core->core_status) {
		...
		trans_dbg(tdev, TR_ERR,
			"vc8000e: %s, core_%d_status:%s err, hw_status=0x%x\n",
			__func__, index, core_status[core->core_status], irq_status);
		spin_unlock(&core->irq_lock);
		return IRQ_NONE;
	}
